...
<body>
    <main class="flex flex-col items-center justify-center h-screen bg-gray-100 dark:bg-gray-900">
        ...
        <section class="flex justify-start w-1/2 rounded-2xl p-10 max-h-max bg-white dark:bg-gray-800">
            <form class="flex flex-col gap-6 w-full text-white" action="{{ route('tasks.store') }}" method="post">
                @csrf
                <div class="flex flex-col gap-2">
                    <label for="title">Título</label>
                    <input class="rounded-2xl text-black bg-gray-300" type="text" name="title" id="title">
                </div>

                <div class="flex flex-col gap-2">
                    <label for="description">Descrição</label>
                    <textarea class="rounded-2xl text-black bg-gray-300" name="description" id="description"></textarea>
                </div>
